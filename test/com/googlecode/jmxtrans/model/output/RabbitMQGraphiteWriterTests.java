...
public class RabbitMQGraphiteWriterTests {
	...
	private static RabbitMQGraphiteWriter getGraphiteWriter(OutputStream out) throws Exception {
		return getGraphiteWriter(out, new ArrayList<String>());
	}

	private static RabbitMQGraphiteWriter getGraphiteWriter(OutputStream out, List<String> typeNames) throws Exception {
		...
		RabbitMQGraphiteWriter writer = RabbitMQGraphiteWriter.builder()
				.setHost("localhost")
				.setPort(2003)
	...
	public void writeSingleResult() throws Exception {
		...
		RabbitMQGraphiteWriter writer = getGraphiteWriter(out);
	...
	public void booleanAsNumberWorks() throws Exception {
		File testInput = new File(RabbitMQGraphiteWriterTests.class.getResource("/booleanTest.json").toURI());
		...
		RabbitMQGraphiteWriter writer = (RabbitMQGraphiteWriter) (query.getOutputWriters().get(0));
		writer.setPool(pool);
	...
	public void checkEmptyTypeNamesAreIgnored() throws Exception {
		...
		typeNames.add("name");
		typeNames.add("type");
		RabbitMQGraphiteWriter writer = getGraphiteWriter(out, typeNames);
